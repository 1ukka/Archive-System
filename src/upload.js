    ...
    if (file) {
      ...
      reader.onload = function (ev) {
        setFormData((prev) => ({
          ...prev,
          file: file,
          fileURL: ev.target.result, // Store the file URL to display it
        }));
      };
  ...
  return (
    <div>
      <div>
        ...
        {formData.fileURL && (
        <div className="fileReviewer">
          <iframe src={formData.fileURL} style={{ width: '100%', height: '500px' }} frameBorder="0"></iframe>
        </div>
      )}
      </div>
    </div>
